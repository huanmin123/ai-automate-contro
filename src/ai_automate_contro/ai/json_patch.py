...
def apply_json_patch_operations(document: Any, operations: list[dict[str, Any]]) -> Any:
    ...
    for operation in operations:
        ...
        if op_name == "replace":
            ...
            raise ValueError(f"不支持的 JSON patch 操作：{op_name}")
...
def _replace_json_path(document: Any, path: list[Any], value: Any) -> Any:
    ...
    if isinstance(parent, dict):
        ...
        if key not in parent:
            raise KeyError(f"JSON path 不存在：{format_json_path(path)}")
        parent[key] = value
        return document
    if isinstance(parent, list):
        ...
        parent[index] = value
        return document
    raise TypeError(f"JSON path 父节点不是容器：{format_json_path(path[:-1])}")
...
def _remove_json_path(document: Any, path: list[Any]) -> Any:
    if not path:
        raise ValueError("不支持删除 JSON 文档根节点。")
    parent, segment = _resolve_json_parent(document, path)
    if isinstance(parent, dict):
        key = str(segment)
        if key not in parent:
    ...
    for depth, segment in enumerate(path[:-1], start=1):
        if isinstance(current, dict):
            ...
            if key not in current:
                raise KeyError(f"JSON path 不存在：{format_json_path(path[:depth])}")
            current = current[key]
            continue
        if isinstance(current, list):
            ...
            continue
        raise TypeError(f"JSON path 片段不可达：{format_json_path(path[:depth])}")
