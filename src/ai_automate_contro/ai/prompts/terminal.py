...
- 桌面 App/窗口/系统文件对话框定位优先读 `handbook/actions/desktop/app_profile.md`；`desktop_app` 启动字段读 `handbook/actions/desktop/desktop_app.md`。
...
- 常见 App、系统窗口和文件对话框优先用 `profile`：内置 profile 包括 `notepad`、`explorer`、`powershell`、`file_dialog_open`、`file_dialog_save`、`textedit`；业务 App 没有内置 profile 时，优先在当前 plan 包 `config.json` 的 `desktop_profiles` 写 profile，再在 steps 里引用。step 上只写用于缩窄当前任务的覆盖字段，例如临时窗口标题；不要在每一步重复硬编码 `app`、`process_name` 或通用对话框 class。
