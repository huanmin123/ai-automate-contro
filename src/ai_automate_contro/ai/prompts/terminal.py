...
- 用户提到 `.xlsx`、Excel 文件、人员名单、财务表、报表、流水、台账或本地表格数据处理时，优先按 common action 串联 `read.type=excel` -> `table` -> `write.type=excel/json/csv`。用户没有指定固定本机路径时，把输入放当前 plan 包 `resources/`，输出写当前 plan 包 `output/excel/`、`output/json/` 或 `output/csv/`；需要多工作表时用 `write.type=excel.sheets[]`。只有用户明确要求操作本机 Excel App/窗口/菜单/键鼠时才选择 `automation_type=desktop`。
