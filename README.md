...
- 输入 `/` 会打开常用命令候选，Up / Down 选择，Tab 或 Enter 补全；空闲时只展示 AI 客户端入口，例如 `/status`、`/plan`、`/sessions`、`/resume`、`/new`、`/image`、`/export`、`/clear`、`/copy-last` 和 `/exit`。等待审批时才显示 `/approve` 和 `/reject`。`/plan` 只查看 AI 当前工作计划，不负责运行 plan 包；无 AI 的 list/create/validate/run/debug 统一使用 `cplan`。命令没有重复别名；模型连通性排查使用 `python .\main.py ai check --json`。
