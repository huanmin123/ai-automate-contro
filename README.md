...
- 输入区是无边框灰底多行 composer：Enter 发送；在行尾输入 `\` 后按 Enter 可续写下一行；多行粘贴会保留换行。高度随内容在 4 到 8 行内增长。AI 忙碌时继续发送会进入队列；按 Esc 会中断当前 AI/plan，已有队列时把排队内容作为下一轮介入消息优先处理。
- 输入 `/` 会打开命令候选，Up / Down 选择，Tab 或 Enter 补全；可用 `/check` 发送一次真实模型诊断请求，`/details` 切换工具细节，`/export [path]` 导出当前可见对话，`/clear` 清屏，`/copy-last [path]` 保存最近一条 AI 回复。
