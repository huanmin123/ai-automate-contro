...
默认入口会打开 Textual AI 客户端。直接描述需求，客户端会把用户消息、AI 回复、工具进度和审批状态分块显示；无 AI 的 plan 控制使用单独的 `cplan` CLI：
...
- `main.py`: AI-first 启动入口，负责把项目 `src/` 加入导入路径并交给应用层分发。
...
- `resources/`: 本需求独占输入资源。
- `output/`: 本需求运行输出，由 Git 忽略。截图、录屏、下载、日志、报告和运行中间产物都必须写在这里；如果用户明确要求最终文件放到 `Downloads`、桌面或绝对路径，AI 终端会在运行成功后把整理好的产物导出过去。
- `docs/`: 本需求说明文档。
...
- 工具调研、执行进度、系统输出、审批等待和错误会作为独立消息块显示，避免长时间只有空白等待；工具块完成后原地更新，不刷屏。
- 输入区是无边框灰底多行 composer：Enter 发送；`Shift+Enter`、`Alt+Enter` 或行尾 `\` 后 Enter 插入换行；多行粘贴保留换行。高度随内容在 4 到 8 行内增长。AI 忙碌时继续发送会进入队列；按 Esc 会中断当前 AI/plan，已有队列时把排队内容作为下一轮介入消息优先处理。
- 输入 `/` 会打开常用命令候选，Up / Down 选择，Tab 或 Enter 补全；空闲时只展示 AI 客户端入口，例如 `/status`、`/plan`、`/sessions`、`/resume`、`/new`、`/image`、`/export`、`/clear`、`/copy-last` 和 `/exit`。等待审批时才显示 `/approve` 和 `/reject`。`/plan` 只查看 AI 当前工作计划，不负责运行 plan 包；无 AI 的 list/create/validate/run/schedule/debug 统一使用 `cplan`。命令没有重复别名；模型连通性排查使用 `python .\main.py ai check --json`。
- `/export` 和 `/copy-last` 会保存当前对话原文，不自动脱敏。项目默认保留账号、密码、token、业务数据、页面截图说明和模型错误原文，方便本地排查。
- 底部状态条会持续显示当前服务、thread、plan、debug workspace、最近 output、运行状态、队列数量、审批和待发送图片。
- `python .\main.py ai check --json` 会访问真实模型服务，用于区分“本地配置可读”和“服务当前可用”；`python .\main.py ai ask --message "<text>" --json` 用于脚本化 AI 回归。
- 确定性 plan 命令统一走 `python .\cplan.py ...`；AI 工具诊断仍走 `python .\main.py tool ...`，AI 客户端自检仍走 `python .\main.py self-check ...`。

## AI 终端

- AI 终端使用 `langchain.agents.create_agent`、LangChain `StructuredTool`、显式 Pydantic schema 和 LangGraph checkpoint。
- 模型服务从当前运行根的 `default_ai_config_dir/config.json` 读取，服务名默认是 `ai_services.default`。配置可以使用 `api_key` 或 `api_key_env`；工具、日志和调试产物不因为密钥字段而自动替换或拒写。
...
python .\cplan.py self-check data-components
```
...
- `table`: 通过 `type` 对行数组执行筛选、选列、排序、去重、分组、连接和派生列
