...
跨平台桌面控制是规划中的第二执行线。落地后主 `plan.json` 必须通过 `automation_type` 声明 `browser` 或 `desktop`，执行器按类型选择解析路径；浏览器 action 和桌面 action 必须隔离，只共享通用 action、变量、输出和控制流。
...
- 主 plan 需要明确执行线。桌面线落地后顶层 `automation_type` 必填，只能是 `browser` 或 `desktop`；子计划继承主 plan 类型，不能跨线混用浏览器和桌面专属 action。
...
- `plans/**/output/`、`test-plans/**/output/`、`plans/**/profiles/`、`test-plans/**/profiles/`、`.keygen/`、`__pycache__/`、IDE 配置和本地密钥配置属于本地产物，应由 `.gitignore` 过滤，不作为常规提交内容。
...
- 允许提交源码、文档、手册和可复现示例。
- 不提交任何 plan 包下的 `output/`、`profiles/` 目录或 `.keygen/` 本地产物。
