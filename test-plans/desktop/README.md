...
该入口会创建临时 plan 包和临时资源文件。Windows 当前启动受控 WinForms 编辑器、Explorer、可见 PowerShell 终端和临时 WinForms Open/Save 文件对话框，macOS 启动 TextEdit、Finder 和 Swift/AppKit harness，覆盖启动后等待窗口、活动窗口读取、窗口查询、聚焦、控件列表/定位、输入、保存、终端命令执行、真实 `NSOpenPanel`/`NSSavePanel` 默认打开/保存流、对话框截图、结果文件、关闭和关闭后 `not_exists`。
...
`desktop-scenario-apps` 在 Windows 上动态生成受控 WinForms mock chat、mock game、mock recovery 和 mock interference 窗口，在 macOS 上动态生成 Swift/Cocoa mock chat、mock game、mock recovery、mock interference 和受控 AppKit Open/Save 文件对话框窗口，运行真实 desktop plan，覆盖窗口启动、聚焦、控件操作、消息发送、游戏奖励/副本、最小化恢复、阻塞弹窗、同标题多窗口、topmost 遮挡、用户抢焦点、目标窗口移动、`window_id` 锁定、`desktop_input target=element_center` 点击归属、截图证据、状态断言、结果文件、关闭链路和 plan 合同检查。
