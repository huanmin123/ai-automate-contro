...
源码开发环境默认不安装服务型数据库驱动。运行到对应连接时才会加载驱动，用户按需要选择安装：
...
发行包按打包边界原则处理数据库能力：支持的数据库 Python 驱动必须在打包 Python 环境中安装并进入 `_internal/`，不能依赖用户在发行后通过 pip 补装。

SQLite 使用 Python 内置驱动，不需要额外依赖。源码默认安装不会拉取 PostgreSQL、MySQL/MariaDB、Oracle、SQL Server、MongoDB 或 Redis 驱动。
...
探测连接里的表、列和索引元数据。SQLite/PostgreSQL/MySQL/SQL Server/Oracle 均支持表和列；SQLite 额外支持索引明细。
...
- 事务成功自动提交，失败自动回滚。
- 事务子步骤不写 `action`、`connection`、`commit`、`output.as`、`output`、`rows_path` 或 `result_path`；需要给后续节点使用时在父级 `sql` step 写 `output`。
