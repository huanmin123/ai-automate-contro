...
- `stream`: `copy` 是否启用分批游标复制，默认 `false`。
- `fetch_size`: `copy stream=true` 时每次从源游标读取的行数。
...
`import` 返回输入文件、目标表、输入行数、影响行数、批次数、列映射和 `elapsed_ms`。
`export` 返回目标文件、输出格式、列名、导出行数、首行和 `elapsed_ms`。
