...
数据库 common action 已覆盖 SQLite、PostgreSQL、MySQL/MariaDB、Oracle、Redis 和 SQL copy/transaction。后续需要补齐常见数据库生态能力，同时继续保持源码默认安装轻量；发行包按支持能力把数据库驱动打入。
...
- 源码默认安装不新增数据库驱动；发行包需要支持的数据库驱动必须随包进入 `_internal/`。
- SQLite 仍是本地自动化默认推荐。
