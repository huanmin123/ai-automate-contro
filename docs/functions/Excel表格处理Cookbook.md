...
`records` 模式下 `offset_rows` 从表头后的数据行开始计算；`meta.offset_rows`、`meta.limit_rows` 和 `meta.truncated` 可用于判断是否继续读下一页。
...
```json
{
  ...
  "type": "union",
  "source": "{{cleaned_rows}}",
  "sources": ["{{normalized_import}}"],
  "columns": ["姓名", "部门编码", "状态", "入职日期", "部门全称"],
  "fill_missing": "",
  "save_as": "union_rows"
}
```
...
`range` 或模板内 `named_range` 写表格主体，`cells` 写标题区、参数区或汇总区；提供 `template_path + range/named_range` 时默认保留模板其它区域、复制模板行样式并延展相交条件格式。
