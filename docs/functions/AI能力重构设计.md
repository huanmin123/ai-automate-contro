...
- 不把真实密钥写入要分发或提交的配置；真实服务只保留在本机运行根配置或本地私有配置中。
...
除受保护工具外，同一组工具也可以通过 `python .\main.py tool call <name> --args-json '{...}'` 调用，便于真实回归和后续替换 agent 框架。`apply_debug_patch_after_approval` 只能走 AI 终端 HITL 审批恢复流程，或无 AI 场景下走 `cplan debug-apply --yes`。
...
可提交或分发的示例配置只能放服务别名、模型名、超时等非敏感字段。
