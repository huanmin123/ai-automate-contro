...
- `desktop_input target=candidate` 缺少候选来源或 `candidate_id` 失败；候选来源可以是 `target_candidates`、`candidate_source` 或单个 `candidate`；同时展开 `bounds`、Window Query 或 Element Locator 失败。
