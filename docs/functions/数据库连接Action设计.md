...
- `sql`: 关系型数据库。SQLite 是本地化优先路径，另覆盖 PostgreSQL、MySQL/MariaDB、Oracle、SQL Server。
...
数据库能力按“源码按需、发行包随能力打入”落地：

- SQLite 使用 Python 标准库 `sqlite3`，不需要额外安装。
- 源码开发默认安装不包含服务型数据库驱动；需要连接对应服务时按 extra 安装。
- PostgreSQL、MySQL/MariaDB、Oracle、SQL Server、MongoDB 和 Redis 的 action 代码保留在项目中，驱动只在运行到对应连接或 action 时懒加载。
- 发行包如果声明支持数据库能力，相关 Python 驱动必须在打包 Python 环境中安装并进入 `_internal/`，不能依赖用户在发行后通过 pip 补装。
- 缺少驱动时，运行时返回对应库的安装命令，不提示用户安装整套数据库依赖。
- 聚合 extra 作为源码开发和打包环境准备的便利入口。
...
- 源码开发环境按连接类型安装 `db-postgresql`、`db-mysql`、`db-oracle`、`db-sqlserver`、`db-mongodb` 或 `db-redis`；发行包需要支持这些能力时必须在打包环境提前安装并随包进入 `_internal/`。
