...
如果后续要支持 `.xls`、`.xlsb`，再按格式增加可选 extra，例如 `excel-xls`、`excel-xlsb`。`.xlsx`/`.xlsm` 大文件读取通过 `openpyxl` 只读模式、`offset_rows`、`limit_rows`、`preview_rows` 和 `max_cells` 控制，不把整张表一次性加载进内存。
...
`output.as` 是读取结果变量名；读取结构需要进入后续节点时，应作为读取结果的一部分或单独产物设计，不再使用旁路变量字段。
...
- `type`: 固定为 `excel`
- `path`: 输入文件路径
- `output`: 发布行数组、矩阵或单元格值的声明；`output.as` 是变量名

常用可选字段：

- `sheet`: 工作表名称或从 0 开始的索引，默认第一个工作表。
- `sheets`: 多工作表读取数组；每项可包含 `sheet`、`name`、`range`、`header_row`、`headers`、`mode`、`skip_blank_rows`、`max_rows`、`max_cells`、`preview_rows`、`formula_mode` 和 `date_format`。
...
- `max_cells`: 最大读取单元格数，防止超大区域爆内存，默认 `500000`。
- `offset_rows`: 从数据区跳过 N 行后开始读；`records` 模式从表头后的数据行开始计算，`matrix`/`cells` 从范围首行开始计算。
- `limit_rows`: 本次最多读取 N 行，用于分页处理大表。
- `preview_rows`: 只读取前 N 行数据并在 meta 标记 `truncated`，适合 AI 先看 sheet、headers 和样例行再决定后续 plan。
...
`range` 用于写入表格主体，`cells` 用于标题区、参数区或汇总区。模板中未覆盖的区域会保留原有样式和内容。

也可以用模板工作簿的命名区域指定写入范围：
...
`named_range` 不能和 `range` 同时使用。写入模板区域时默认复制样式源行，并把相交的条件格式延展到实际写入数据范围；写入合并单元格时只允许写合并区域左上角。
...
- `write_mode`: `create`、`replace_sheet`、`append_rows`、`overlay_cells`，默认 `create`；有模板且指定 `range`/`named_range` 或仅填充 `cells` 时默认 `overlay_cells`，有模板且写整表 `value` 时默认 `replace_sheet`。
- `start_cell`: 表格写入左上角单元格，默认 `A1`。`freeze_header`、`auto_filter`、`table`、`number_format` 和 `column_widths` 会按实际表格范围应用。
- `range`: 表格写入 A1 区域，例如 `B4:H20`；运行时校验数据不能写出区域。提供 `template_path + range` 且未指定 `write_mode` 时，默认 `overlay_cells`，用于保留模板标题区、汇总区和样式。
- `named_range`: 模板工作簿中的命名区域，作为写入范围；不能和 `range` 同时使用。
- `formula_columns`: 追加公式列，只支持字典行数组；公式可用 `{row}` 和 `{列名}` 占位引用当前行单元格，例如 `"合计": "={金额}+{税额}"`。
- `copy_row_style`: 写模板区域时把样式源行复制到写入数据行；模板 + `range`/`named_range` 默认开启。
- `style_source_row`: 指定复制样式的源行号，默认使用写入区域首个数据行。
- `extend_conditional_formatting`: 写模板区域时把相交条件格式延展到实际写入数据范围；模板 + `range`/`named_range` 默认开启。
...
- `sheets`: 多工作表写入配置，每项可包含 `sheet`、`value`/`rows`、`cells`、`headers`、`start_cell`、`range`、`formula_columns`、`write_mode`、`freeze_header`、`auto_filter`、`table`、`number_format` 和 `column_widths`。
...
当前 `table` 已落地 `filter`、`select`、`sort`、`dedupe`、`group`、`join`、`add_column`、`rename`、`fill_empty`、`type_convert`、`pivot`、`replace`、`split_column`、`merge_columns`、`date_parse`、`lookup`、`normalize_headers`、`union` 和 `fuzzy_lookup`。窗口函数、复杂多表查询或大数据量处理仍优先通过 `sql` + SQLite 或服务型数据库完成。

## AI 生成原则

- “财务流水、人员名单、台账、报表模板”等词只说明可能的数据形态，不代表固定处理规则。
- AI 生成最终 plan 前，应先从用户明确要求或 workbook 预览结构中确定 sheet、headers、样例行、行数、模板区域、筛选条件、汇总维度、连接 key、公式列和输出位置。
- 如果用户只说“整理一下”“按常见报表输出”“自动处理”，不能因为场景词硬套 filter/group/pivot/join；应先追问规则，或先生成只读预览 plan。
- 只读预览 plan 应使用 `read.type=excel` + `preview_rows` + `max_cells` + `output`，并用 `write.type=json` 输出 preview rows；预览不是最终业务处理，不能混入猜测出来的筛选、汇总或公式逻辑。
...
- 使用 `openpyxl.load_workbook(path, read_only=True, data_only=...)`。
- `formula_mode=cached` 时使用 `data_only=True`。
- `formula_mode=formula` 时使用 `data_only=False`。
- 日期、时间、Decimal 等值要转成 JSON 可序列化值。
- 无显式 `range` 时信任工作簿维度，不做全表扫描式边界收紧；分页读取只按表头行和当前窗口计算 `max_cells`。
...
- `range` 写入要同时约束行数和列数，避免覆盖模板外区域。
- `named_range` 先解析为单一 sheet 的 A1 范围，再按 `range` 约束写入。
- 模板区域写入检查合并单元格，不允许写入非左上角 merged cell。
- 模板区域写入可复制数据行样式，并把相交条件格式延展到实际写入数据范围。
- 字典数组按 headers 写列；二维数组直接按矩阵写。
- 公式以 `=` 开头写入单元格，但不计算公式结果；`formula_columns` 负责生成逐行公式文本。
...
- `sheet` 允许字符串或非负整数。
- `range` 必须是 A1 风格字符串。
- `max_cells`、`preview_rows` 必须是正整数。
- `offset_rows` 必须是非负整数，`limit_rows` 必须是正整数。
...
- `formula_columns` 必须是对象，每项是公式字符串或包含 `formula` 的对象。
- `named_range` 和 `range` 不能同时提供。
- `copy_row_style`、`extend_conditional_formatting` 必须是布尔值，`style_source_row` 必须是正整数。
...
`test-plans/data-driven/table-cleaning/` 覆盖常见清洗操作：

- `replace`、`split_column`、`merge_columns`、`date_parse` 和 `lookup`。
- `normalize_headers`、`union` 和 `fuzzy_lookup`。
- 使用内置变量，不依赖外部文件，便于快速验证清洗链路。
...
动态 data-components 自检额外覆盖：

- 模板工作簿样式保留、指定 `range` 写入、标题区/汇总区 `cells` 写入和 `formula_columns`。
- 模板命名区域 `named_range`、样式源行复制、条件格式延展和合并单元格写入保护。
- 运行时负例：不存在的 `named_range`、命名区域容量不足、写入合并单元格非左上角。
- 临时生成万行级 `.xlsx` fixture，覆盖 `offset_rows`/`limit_rows` + `max_cells` 的分页读取，验证中间页和尾页不因跳过行过多而触发超大范围读取。
- `preview_rows`/`offset_rows`/`limit_rows` + `max_cells` 的大文件预览和分页保护，以及 `meta.truncated` 标记。
