...
当前 `table` 已落地 `filter`、`select`、`sort`、`dedupe`、`group`、`join`、`add_column`、`rename`、`fill_empty`、`type_convert`、`pivot`、`replace`、`split_column`、`merge_columns`、`date_parse` 和 `lookup`。窗口函数、复杂多表查询或大数据量处理仍优先通过 `sql` + SQLite/DuckDB 完成。
...
- `preview_rows` + `max_cells` 的大文件预览保护和 `meta.truncated` 标记。
