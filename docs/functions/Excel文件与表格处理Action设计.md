...
当前 `table` 已落地 `filter`、`select`、`sort`、`dedupe`、`group`、`join`、`add_column`、`rename`、`fill_empty`、`type_convert`、`pivot`、`replace`、`split_column`、`merge_columns`、`date_parse`、`lookup`、`normalize_headers`、`union` 和 `fuzzy_lookup`。窗口函数、复杂多表查询或大数据量处理仍优先通过 `sql` + SQLite/DuckDB 完成。
...
- 只读预览 plan 应使用 `read.type=excel` + `preview_rows` + `max_cells` + `save_meta_as`，并用 `write.type=json` 输出 preview rows/meta；预览不是最终业务处理，不能混入猜测出来的筛选、汇总或公式逻辑。
