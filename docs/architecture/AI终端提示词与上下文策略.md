...
- 执行线：目标是浏览器网页自动化，还是本机桌面应用/系统级键鼠控制。
...
- desktop 线用桌面窗口列表、截图、状态快照、权限诊断、控件/图像证据或轻量探索 plan 降低不确定性；不要把桌面取证写成网页探测，也不要要求 `open_browser` 或 `navigate`。
...
- 用户没有明确说明执行线，且需求既可能是网页也可能是桌面应用；此时必须先确认 `automation_type`，不能边写 plan 边猜。
...
1. 能用上下文直接确定：直接执行。
2. 能用只读工具确定：先取证，再执行。
3. 执行线不明确：先问用户确认 browser 还是 desktop，再进入对应 handbook。
4. 能创建探索 plan 降低不确定性：先创建并运行探索 plan；浏览器需要人工动作时，必须停在同一个可见 Playwright 浏览器窗口里交接；桌面需要授权或人工动作时，必须说明当前桌面窗口、权限缺口和继续方式。
5. 需要用户授权、一次性验证码、二次验证、外部设备确认或业务选择：开工前问清楚，执行时通过对应执行线承接。
6. 执行中出现新的安全门槛、页面状态变化或桌面权限缺口：暂停并说明当前证据、缺口和用户要在哪里完成操作。
...
- 保留真实网页最终 plan 创建前必须 `inspect_web_page` 取入口证据、必要时运行 headed 探索 plan 的硬规则；同时新增 desktop plan 的窗口/截图/权限/状态证据规则，质量门禁按 `automation_type` 分流。
