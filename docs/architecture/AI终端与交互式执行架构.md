...
- 执行线取证和质量门禁：browser plan 面向真实网站时，`inspect_web_page` 必须在最终 plan 前优先使用；`review_plan_quality` 会结合最近探测上下文、用户需求和 plan 内容做门禁。desktop plan 不要求 `inspect_web_page`、`open_browser` 或 `navigate`，质量门禁使用 `open_desktop`、窗口列表、截图、状态快照、权限诊断、桌面断言和运行产物做证据。
