...
- `export_local_file`：把最终交付物写入用户明确指定的本机路径，或把当前 plan `output/` 下的产物复制过去。plan 运行证据仍留在 `output/`；Downloads、桌面或绝对路径这类最终交付不要求用户手动复制。
- `validate_plan`
- `run_plan`
...
- `create_debug_workspace`：已完成底层工具；无 AI 人工调试走 `cplan debug-create`。
- `list_output_artifacts`：已完成底层工具；Textual 客户端不再暴露产物浏览管理命令。
...
- `inject_debug_steps`：已完成底层工具；无 AI 人工调试走 `cplan debug-inject`。
- `write_debug_workspace_file`：已完成受限写入，只能写 debug workspace 内的 `injected-plan/`、`notes.md` 和 `report.md`。
- `validate_debug_plan`
- `run_debug_plan`
- `generate_debug_patch`：已完成底层工具；无 AI 人工调试走 `cplan debug-patch`。
