...
1. 询问缺失信息。
2. 判定执行线：`browser` 或 `desktop`。
3. 不确定执行线时向用户确认，不能直接写 plan。
4. 创建 plan 包。
5. 写入带 `automation_type` 的 `plan.json`、`config.json`、`docs/README.md`。
6. 运行校验。
7. 询问用户是否运行。
8. 运行 plan。
9. 读取日志、输出和失败证据。
...
AI 终端创建或大改 plan 前，必须先得到明确执行线，并写入 `automation_type`：

- `browser`: 网页、URL、浏览器、Chrome 页面、后台网页、DOM、网页表单、网页按钮、网页下载、浏览器 storage state。
- `desktop`: 本机桌面应用、系统窗口、菜单栏、Finder、TextEdit、Notepad、Excel 桌面版、系统设置、桌面键盘鼠标、截图识别、OCR、macOS 权限授权。

用户描述模糊时，AI 终端必须追问。例如“帮我自动登录后台”“操作表格”“打开微信”“处理客户端”都可能是网页或桌面应用，不能直接选择。推荐追问：

```text
你要自动化的是网页里的流程，还是本机桌面应用里的流程？两者的 plan 类型不同。
```

只有用户非常明确表达网页或桌面时，AI 才能跳过追问。确认后：

- browser plan 读取 [handbook/browser/README.md](../../handbook/browser/README.md)，只能写浏览器专属 action 和通用 action。
- desktop plan 读取 [handbook/desktop/README.md](../../handbook/desktop/README.md)，只能写桌面专属 action 和通用 action。

执行线判定不能只依赖 action 名称反推。`cplan validate/run` 也不能猜测类型，必须读取顶层 `automation_type`，详见 [执行线隔离与Plan类型设计](./执行线隔离与Plan类型设计.md)。
...
- 执行线取证和质量门禁：browser plan 面向真实网站时，`inspect_web_page` 必须在最终 plan 前优先使用；desktop plan 面向真实桌面 App 时，`inspect_desktop` 应优先用于获取平台、backend、`capability_matrix`、权限/依赖、窗口列表、可选控件树摘要和截图路径；`review_plan_quality` 会结合最近探测上下文、用户需求和 plan 内容做门禁。desktop plan 不要求 `inspect_web_page`、`open_browser` 或 `navigate`，质量门禁使用 `inspect_desktop` 摘要、`open_desktop`、`capability_matrix`、`desktop_app`、窗口列表、控件列表/dump/定位/文本/状态、控件断言、截图、状态快照、权限诊断、桌面标注、桌面断言和运行产物做证据；App 启动、窗口生命周期控制和 `desktop_element click/set_text/select/invoke` 本身不能替代窗口、控件、截图、等待或断言证据。
