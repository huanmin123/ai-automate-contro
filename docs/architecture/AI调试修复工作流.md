...
- 需要用户协助的登录、验证码、二次验证等步骤提前告知，并通过管理终端等待/继续。
...
这些命令先服务管理终端，后续 AI 终端通过同一套函数调用，不重新实现文件复制和产物扫描逻辑。
...
- `manual_confirm`：需要用户登录、输入账号密码、处理验证码或确认页面状态时等待。
...
AI 终端应用补丁还必须经过 LangChain `HumanInTheLoopMiddleware`。模型请求 `apply_debug_patch_after_approval` 时会先中断并显示 `[WAIT_APPROVAL]`，用户只能用 `approve` 或 `reject <reason>` 恢复。工具包装层还有二次守卫：只有 `approve` 恢复期间注入的 `approved: true` 才能真正执行补丁应用，普通对话里出现“同意”“批准”等文字不能绕过审批。这个受保护工具也不能通过 `python .\main.py tool call ...` 直接调用；脚本显式应用补丁必须使用 `python .\main.py plan debug-apply --workspace <output/debug/run> --yes`。
...
plan 中使用 `manual_confirm` 进入等待态，管理终端输出等待原因。用户完成操作后回到终端执行 `continue`。
