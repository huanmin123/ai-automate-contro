...
AI 终端工具通常是 plan 级能力，只服务创建、管理、运行、调试、修复和报告 plan。`run_local_command` 是直接操作本机终端的例外：它不是 plan action，不能写入 `steps`，但可供 AI 安装依赖、调用包管理器或执行任意本机命令。
...
8. 对可直接 CLI 调用的工具运行 `python .\main.py tool call <tool-name> --args-json '{...}'` 做回归；`run_plan` 必须通过 AI 终端质量门禁后的工具循环验证，`apply_debug_patch_after_approval` 必须通过 HITL 审批流程验证。
...
- `run_local_command` 不对命令文本、`argv`、工作目录、环境变量、本机路径、包管理器或 stdout/stderr 施加策略过滤；其 `timeout_seconds <= 0` 表示不设超时。非零退出码、超时和启动失败必须返回结构化原始结果，供 AI 继续诊断。
...
- 为真实桌面 App、窗口、系统菜单或键鼠流程创建最终 plan 时，AI 终端优先调用 `inspect_desktop` 获取平台、backend、`capability_matrix`、权限/依赖、窗口列表、可选控件树摘要和截图路径；`inspect_desktop` 是 plan 级只读工具，不写入 `steps`，最终 plan 仍必须通过 `open_desktop`、窗口/控件/截图/等待/断言产物留运行证据。
...
```powershell
python .\main.py tool schema inspect_desktop
python .\main.py tool call inspect_desktop --args-json '{"platform_name":"auto","request_permissions":false,"include_windows":true,"include_elements":false,"max_windows":5}' --compact
```
