...
1. 在 `src/keygen_automation/ai_terminal_tools.py` 中实现确定性工具函数，返回 JSON 可序列化的 `dict`。
...
5. 在 `src/keygen_automation/ai_terminal_tool_schemas.py` 中新增显式 Pydantic 参数模型，并加入 `TOOL_ARGS_SCHEMAS`。
...
```powershell
python -m py_compile .\src\keygen_automation\ai_terminal_tool_schemas.py .\src\keygen_automation\ai_terminal_langgraph.py .\src\keygen_automation\ai_terminal_tools.py .\src\keygen_automation\ai_terminal.py .\src\keygen_automation\cli.py
