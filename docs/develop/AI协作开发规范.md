...
- 优化真实浏览器流程时先读 `output/<run>/events.jsonl` 或 `run.log`，按 step 计算耗时，再判断是浏览器启动、首次导航、固定等待、条件等待、接口响应、下载、文件上传还是大表提取慢。不要凭感觉加长 sleep 或开启隐式截图；优先修正 locator、等待条件、事件 `trigger`、登录态复用和浏览器会话复用。
...
- 优化真实桌面流程时先读 `output/<run>/events.jsonl` 或 `run.log`，按 step 计算耗时，再判断是启动、窗口归一化、固定等待、前台保护还是业务动作慢。不要凭感觉删除等待、关闭前台保护或改成裸坐标；连续同窗口输入优先调 `desktop.foreground_protection.cache_ttl_ms`，并保留必要的窗口/控件/结果断言。
...
- 新增桌面 action、desktop backend、桌面失败采集或桌面校验规则：运行 `python .\cplan.py self-check desktop-components`；涉及 `desktop_vision locate_image` 时确认 OpenCV 模板匹配、`desktop-vision` JSON/原图/裁剪图/标注图回归覆盖；桌面线不支持 OCR 或 `desktop_vision locate_text`，不要新增 Tesseract/OCR 依赖探测、CLI 门禁或 plan action；涉及真实系统 App、焦点、键鼠输入、保存、PowerShell/终端窗口、Explorer/Finder 类系统窗口、Windows common dialog 或 macOS 真实 `NSOpenPanel`/`NSSavePanel` 默认打开/保存流时补跑 `python .\cplan.py self-check desktop-real-app`；涉及 macOS 受控 AppKit Open/Save 弹窗、Swift/Cocoa 场景窗口、干扰和恢复时补跑 `desktop-scenario-apps`；涉及特定第三方 App sheet 形态、任意路径输入或定制文件选择器时仍需专项探测和回归；涉及 `desktop_element dump/set_text/select/invoke`、`desktop_input` 鼠标目标、`desktop_assert element` 或 AI desktop debug 闭环时确认 schema 负例、Windows WinForms 和 macOS Swift/Cocoa 回归覆盖；无 GUI、锁屏、权限不足或依赖缺失时应返回 skipped 和原因。
