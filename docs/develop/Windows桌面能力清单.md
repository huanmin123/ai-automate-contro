...
- 桌面探测：`open_desktop`、`inspect_desktop`、`desktop_capture type=observe/snapshot` 返回 `capability_matrix`、`coordinate_profile`、窗口摘要、候选和诊断。
- 窗口能力：窗口列表、查询、活动窗口读取、聚焦、关闭、最小化、最大化、还原。
- 真实 App：Notepad、Explorer、可见 PowerShell 终端、临时 WinForms Open/Save 文件对话框。
- 控件能力：WinForms TextBox、Button、CheckBox、ComboBox、ListBox、DataGridView、TreeView、MenuStrip、ContextMenuStrip、滚动 Panel。
...
- 输入能力：键盘、快捷键、剪贴板输入、鼠标点击、双击、右键、滚轮、拖拽、`current_window_offset`、`element_center`、`bounds_center`、`target=candidate`。
...
- 组件和 OCR：`python .\cplan.py self-check desktop-components --require-vision --require-ocr --require-ocr-zh`
...
- 真实 AI 回归只验证模型按 AI 终端规则调用工具和判断执行线，不承担 Notepad/Explorer/WinForms/OCR 的重型桌面矩阵；这些由确定性自检负责。
